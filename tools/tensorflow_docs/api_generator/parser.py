...
class _FileLocation(object):
  ...
  def url(self) -> Optional[str]:
    if self.start_line and self.end_line and 'github.com' in self.base_url:
...
def is_class_attr(full_name, index):
  ...
  return bool(inspect.isclass(index[parent_name]))
...
class IgnoreLineInBlock(object):
  ...
  def __init__(self, block_start, block_end):
    ...
    self._start_end_regex = (
        f'{re.escape(self._block_start)}.*?{re.escape(self._block_end)}')
...
class ReferenceResolver(object):
  ...
  def from_visitor(cls, visitor, **kwargs):
    ...
    for full_name, obj in visitor.index.items():
      ...
      if obj_type in (ObjType.CLASS, ObjType.MODULE):
        ...
        is_fragment[full_name] = bool(is_class_attr(full_name, visitor.index))
      else:
  ...
  def _partial_symbols(self, symbol):
    ...
    return ['.'.join(split_symbol[i:]) for i in range(1, len(split_symbol) - 1)]
  ...
  def to_json_file(self, filepath):
    ...
    json_dict = {
        ...
        if key not in ['_all_names', '_partial_symbols_dict']
    }
...
def _get_other_member_doc(
  ...
  description = my_doc if my_doc != class_doc and my_doc is not None else None
  ...
  return info if description is None else f'{info}\n\n{description}'
...
class FormatArguments(object):
  ...
  def _get_non_builtin_ast_types(self, ast_typehint: str) -> List[str]:
    ...
    return [
        single_type
        for single_type, _ in self._INDIVIDUAL_TYPES_RE.findall(ast_typehint) if
        single_type and single_type not in self._TYPING and single_type != '...'
    ]
...
def generate_signature(
  ...
  if varargs is not None:
    all_args_list.insert(varargs[0], f'*{varargs[1].name}')

  if varkwargs is not None:
    all_args_list.append(f'**{varkwargs.name}')
...
def _get_defining_class(py_class, name):
  return next(
      (cls for cls in inspect.getmro(py_class) if name in cls.__dict__), None)
...
class ClassPageInfo(PageInfo):
  ...
  def _add_property(self, member_info: MemberInfo):
    ...
    new_parts = [
        ...
        *[
            str(part)
            for part in doc.docstring_parts if not isinstance(part, TitleBlock)
        ],
        '',
    ]
    desc = '\n'.join(new_parts)
  ...
  def _add_method(
    ...
    if isinstance(original_method, classmethod):
      func_type = FuncType.CLASSMETHOD
          ...
          or not isinstance(original_method, staticmethod) and not is_dataclass):
  ...
  def _add_member(
    ...
    if obj_type is ObjType.PROPERTY:
      ...
      if issubclass(self.py_object, ProtoMessage) and (
          (member_info.short_name.endswith('_FIELD_NUMBER')
...
class ModulePageInfo(PageInfo):
  ...
  def collect_docs(self, parser_config):
    ...
    for member_short_name in member_names:
      ...
      if self.full_name:
        member_full_name = f'{self.full_name}.{member_short_name}'
      else:
...
def _get_defined_in(py_object: Any,
  ...
  if rel_path.endswith('.pyc'):
    ...
    rel_path = rel_path.replace(f'__pycache__{os.sep}', '')
...
def generate_global_index(library_name, index, reference_resolver):
  ...
  for full_name, py_object in index.items():
    ...
    if obj_type is ObjType.CALLABLE and is_class_attr(full_name, index):
      continue
    symbol_links.append(
        (full_name, reference_resolver.python_link(full_name, full_name, '..')))

  lines = [
      f'# All symbols in {library_name}',
      '',
  ...
  lines.extend(f'*  {link}' for link in primary_symbol_links)
  if compat_v2_symbol_links:
    lines.append('\n## Compat v2 symbols\n')
    lines.extend(f'*  {link}' for link in compat_v2_symbol_links)
  if compat_v1_symbol_links:
    lines.append('\n## Compat v1 symbols\n')
    lines.extend(f'*  {link}' for link in compat_v1_symbol_links)
